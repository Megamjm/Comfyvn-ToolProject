...
document.getElementById("rebuildBtn").onclick = async () => {
  const r = await api("/api/export_renpy", "POST");
  if (r.ok) {
    alert("Routes rebuilt:\n" + (r.routes ? r.routes.join("\n") : r.scenes.join("\n")));
  } else {
...
api("/api/config").then(cfg => {
  pollInterval = (cfg.poll_interval_seconds || 3) * 1000;
...
themeSelect.onchange = () => {
  ...
  api("/api/config", "POST", { ui_theme: themeSelect.value });
};

// Load theme from config
api("/api/config").then(cfg => {
  const mode = cfg.ui_theme || "theme-dark";
