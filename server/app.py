...
import os, json, sqlite3, time, uuid, base64
...
DATA_DIR = os.environ.get("VN_DATA_DIR", os.path.join(os.path.dirname(__file__), "..", "data"))
...
def queue():
    ...
    with db() as conn:
        conn.execute(
            "INSERT INTO assets (id, status, title, created_at, updated_at, png_path, json_sidecar_path, meta_json, tags) VALUES (?,?,?,?,?,?,?,?,?)",
    ...
    try:
        ...
        comfy_resp = r.json() if r.headers.get("content-type","").startswith("application/json") else {"text": r.text}
    except Exception as e:
        comfy_resp = {"error": str(e)}

    return jsonify({"id": asset_id, "queued": True, "forwarded_to_comfy": comfy_ok, "comfy_response": comfy_resp})
...
def ingest(asset_id: str):
    ...
    png_dir = os.path.join("static","assets")
    os.makedirs(png_dir, exist_ok=True)
    png_path_rel = os.path.join(png_dir, f"{asset_id}.png")
    img.save(os.path.join(os.path.dirname(__file__), png_path_rel))

    if 'sidecar' in request.files:
        side = request.files['sidecar']
        side_dir = os.path.join("static","assets")
        os.makedirs(side_dir, exist_ok=True)
        side.save(os.path.join(os.path.dirname(__file__), side_dir, f"{asset_id}.json"))
...
def status_api():
    # Placeholder ComfyUI check (can be linked to your comfy_client later)
...
previewBtn.title = "Preview scene";
previewBtn.onclick = () => {
  const sceneId = prompt("Enter scene ID to preview:");
  if (sceneId) window.open(`/preview/${sceneId}`, "_blank");
};
...
@app.route("/api/gallery/decision", methods=["POST"])
def gallery_decision():
    data = request.get_json()
    ...
    if os.path.exists(meta_src):
        os.rename(meta_src, os.path.splitext(dst)[0] + ".json")
...
EXPORT_QUEUE_DIR = "data/export_queue"
os.makedirs(SUMMARIES_DIR, exist_ok=True)
os.makedirs(EXPORT_QUEUE_DIR, exist_ok=True)
...
def sync_comfyui():
    """
    Pulls new renders from ComfyUI history (or simulated list)
    and copies metadata into data/gallery/.
    """
    comfy_dir = "ComfyUI/output"  # adjust if needed
    ...
    for png_path in glob.glob(os.path.join(comfy_dir, "*.png")):
        ...
        if not os.path.exists(target):
            try:
                ...
                import shutil; shutil.copy(png_path, target)
            meta = {
                "id": str(uuid.uuid4()),
...
def generate_summary():
    """
    Creates a simple textual summary for a render using metadata and filename.
    """
    data = request.get_json()
    ...
    # Simulated summary logic (replace with real LLM API later)
    summary = f"Scene {meta.get('id', '')[:6]}: Generated image '{filename}' from {meta.get('source', 'unknown source')}."
...
def export_to_queue():
    """
    Adds approved renders to VN export queue for Ren'Py scene generation.
    """
    data = request.get_json()
    filename = data.get("filename")
...
RENPY_PROJECT_DIR = "data/renpy_project"
SCRIPT_DIR = os.path.join(RENPY_PROJECT_DIR, "game", "scripts")
os.makedirs(SCRIPT_DIR, exist_ok=True)


def build_rpy(scene_data):
    """Generate minimal Ren'Py script content."""
    fn = scene_data["filename"]
    label = scene_data["scene_id"]
...
def export_to_renpy():
    ...
    # Generate minimal Ren'Py launcher structure if missing
...
@app.route("/api/launch_vn", methods=["POST"])
def launch_vn():
    """Launch Ren'Py VN project via batch file."""
    if platform.system() == "Windows":
        try:
            subprocess.Popen(["launch_renpy.bat"], shell=True)
...
def launch_renpy():
    """Launch the Ren'Py project via batch file (Windows) or shell (Unix)."""
...
def preview_scene(scene_id):
    """Lightweight HTML viewer for exported scene summaries."""
