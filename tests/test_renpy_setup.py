...
def test_ensure_renpy_sdk_reuses_existing_install(tmp_path: Path):
    ...
    result = renpy_setup.ensure_renpy_sdk(version="9.9.9", install_root=install_root)
...
def test_ensure_renpy_sdk_installs_via_stubbed_archive(tmp_path: Path, monkeypatch):
    ...
    def _fake_download(version: str, _client, *, tmp_dir: Path) -> Path:
        target = tmp_dir / archive.name
        shutil.copy2(archive, target)
        return target

    monkeypatch.setattr(renpy_setup, "_download_archive", _fake_download)

    result = renpy_setup.ensure_renpy_sdk(install_root=install_root, client=client)
