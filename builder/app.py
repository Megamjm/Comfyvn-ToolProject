...
def merge(body: dict):
    ...
    if not phases: return {"ok": False, "error": "no phases"}
    for ph in phases:
        rp = Path("/") / "mnt" / "data" / ph if ph.startswith("ComfyVN_Phase") else Path(ph)
        rp = Path("./") / ph
        if not rp.exists(): return {"ok": False, "error": f"phase not found {ph}"}
        ...
        for p in rp.rglob("*"):
            if p.is_file():
                ...
                shutil.copy2(p, out)
    # Zip
    zpath = Path("./dist"); zpath.mkdir(parents=True, exist_ok=True)
