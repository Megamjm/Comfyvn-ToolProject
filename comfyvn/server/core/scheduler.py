...
SCHED_PATH = Path("./data/scheduler.json")
...
class Enqueuer:
    ...
    def enqueue(
        self, typ: str, payload: Dict[str, Any], retries: Optional[int] = None
    ) -> str:
        r = retries if retries is not None else self.default_retries
        if self.rq:
            return self.rq.enqueue(typ, payload, retries=r)
...
class Scheduler:
                    ...
                    it.update(
                        {
                            ...
                            if k
                            in {"name", "cron", "payload", "type", "enabled", "retries"}
                        }
                    )
    ...
    def run_now(self, sid: str) -> Optional[str]:
        with self.lock:
            for it in self.list():
                if it["id"] == sid:
                    jid = self.enq.enqueue(
                        it["type"], it["payload"], retries=it.get("retries", 0)
                    )
                    it["last_run"] = _now_ts()
                    # keep next_run as is
                    db = _load()
    ...
    def _in_blackout(self, it, now_ts: float) -> bool:
        try:
            ...
            for r in rules:
                ...
                if dow != "*" and dow_map.get(dow, -1) != now.weekday():
                    continue
                s = r.get("start", "00:00")
                e = r.get("end", "23:59")
                hs, ms = [int(x) for x in s.split(":")]
                he, me = [int(x) for x in e.split(":")]
                    ...
                    if nr and now + 0.5 >= nr:
                        ...
                        if self._in_blackout(it, now):
                            ...
                            try:
                                ...
                                j = int(it.get("jitter_sec") or 0)
                                if j > 0:
                                    nxt = nxt + _dt.timedelta(
                                        seconds=random.randint(0, j)
                                    )
                                it["next_run"] = float(nxt.timestamp())
                            except Exception:
                                it["enabled"] = False
                        ...
                        if should_fire:
                            ...
                            if lttl > 0 and conc in {"skip", "queue"}:
                                it["lock_until"] = now + lttl
                        # compute next
                        try:
                            tz = ZoneInfo(it.get("tz") or "UTC")
                            base = _dt.datetime.fromtimestamp(nr, tz=tz)
