...
def enforce(db, token_row, cost: int = 1):
    ...
    def g(name, d=0):
        try:
            return int(getattr(token_row, name))
        except Exception:
            return d

    def s(name, v):
        try:
            setattr(token_row, name, v)
        except Exception:
            pass
    ...
    if g("limit_per_min") and g("used_min") + cost > g("limit_per_min"):
        raise RateLimitError("minute limit exceeded")
    if g("limit_per_day") and g("used_day") + cost > g("limit_per_day"):
        raise RateLimitError("daily limit exceeded")
    if g("quota_total") and g("used_total") + cost > g("quota_total"):
        raise RateLimitError("quota exceeded")

    s("used_min", g("used_min") + cost)
    s("used_day", g("used_day") + cost)
    s("used_total", g("used_total") + cost)
    try:
        db.commit()
    except Exception:
        pass
