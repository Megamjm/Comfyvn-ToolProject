...
ASSETS = Path("data/assets"); ASSETS.mkdir(parents=True, exist_ok=True)
...
def put_bytes(b: bytes, *, kind: str = "generic", meta: Optional[Dict[str,Any]] = None) -> Dict[str,Any]:
    h = _hash_blob(b)
    p = ASSETS / f"{kind}_{h}"
    p.write_bytes(b)
    (ASSETS / f"{p.name}.json").write_text(json.dumps({"hash": h, "kind": kind, "ts": time.time(), "meta": meta or {}}, indent=2), encoding="utf-8")
    return {"ok": True, "key": p.name, "hash": h}

def link_cache(src_key: str, tag: str) -> Dict[str,Any]:
    src = ASSETS / src_key
    if not src.exists(): return {"ok": False, "error": "missing"}
