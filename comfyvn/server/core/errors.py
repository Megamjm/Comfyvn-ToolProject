...
import uuid, logging, time, traceback

_log = logging.getLogger("comfyvn.errors")

def _problem(status: int, title: str, detail: str | None, request: Request, type_: str = "about:blank") -> JSONResponse:
    rid = request.headers.get("X-Request-ID") or getattr(getattr(request, "state", object()), "X-Request-ID", None)
    body = {
        "type": type_,
        "title": title,
        "status": status,
...
def register_exception_handlers(app):
    ...
    async def _unhandled(request: Request, exc: Exception):
        ...
        return _problem(500, "Internal Server Error", f"error_id={err_id}", request, type_="https://example.com/internal-error")