...
DATA_DIR = Path("./data/search"); DATA_DIR.mkdir(parents=True, exist_ok=True)
...
def _load(db_url_env: str | None = None) -> Tuple[List[Dict[str,Any]], List[Dict[str,Any]]]:
    # scenes, characters
    scenes, chars = [], []
    ...
    if use_db:
        try:
            ...
            from comfyvn.server.core.db import get_db, SceneRow, CharacterRow, init_db
            init_db()
            for db in get_db():
                rows = db.query(SceneRow).all()
                for r in rows:
                    try:
                        d = json.loads(r.data or "{}"); d["__updated"] = r.updated; scenes.append(d)
                    except Exception: pass
                rows = db.query(CharacterRow).all()
                for r in rows:
                    try:
                        d = json.loads(r.data or "{}"); d["__updated"] = r.updated; chars.append(d)
                    except Exception: pass
                break
        except Exception:
            use_db = False
    if not use_db:
        ...
        for p in sdir.glob("*.json"):
            try:
                d = json.loads(p.read_text(encoding="utf-8")); d["__updated"] = p.stat().st_mtime
                scenes.append(d)
            except Exception: pass
...
def _doc_from_scene(s: Dict[str,Any]) -> Dict[str,Any]:
    ...
    return dict(doc_id=f"scene:{sid}", dtype="scene", title=title, content=content, tags=",".join(tags), project=str(project or ""), updated=int(s.get("__updated") or time.time()))
...
def _doc_from_char(c: Dict[str,Any]) -> Dict[str,Any]:
    ...
    return dict(doc_id=f"character:{name}", dtype="character", title=name, content=bio, tags=",".join(tags), project=str(project or ""), updated=int(c.get("__updated") or time.time()))
...
def _open_or_create():
    if not INDEX_DIR.exists(): INDEX_DIR.mkdir(parents=True, exist_ok=True)
...
def reindex(full: bool = False) -> Dict[str, Any]:
    ...
    man = {"docs": {}}
    if MANIFEST.exists():
        try: man = json.loads(MANIFEST.read_text(encoding="utf-8"))
        except Exception: man = {"docs": {}}
    ix = _open_or_create()
    w = ix.writer(limitmb=256, procs=1, multisegment=True)
    added = 0; updated = 0
    seen_ids = set()
    for d in docs:
        did = d["doc_id"]; seen_ids.add(did)
        h = _hash(d)
        ...
        try:
            ...
            pass
        w.add_document(**d); man["docs"][did] = h
        added += 1
    w.commit()
...
def facets(query: str = "", filters: Dict[str, Any] | None = None, limit: int = 100) -> Dict[str, Any]:
    filters = filters or {}
    ix = _open_or_create()
    from collections import Counter
    f_type = Counter(); f_tag = Counter(); f_project = Counter()
    with ix.searcher() as s:
        qp = MultifieldParser(["title","content","tags"], schema=ix.schema, group=OrGroup.factory(0.9))
        ...
        for hit in res:
            ...
            if proj: f_project[proj] += 1
    return {"type": f_type.most_common(50), "tag": f_tag.most_common(50), "project": f_project.most_common(50)}
...
def search(body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"ok": True, "items": out, "facets": facets(q, {"types": types, "tags": tags, "project": project})}

# Saved searches
def saved_list() -> Dict[str, Any]:
    try: data = json.loads(SAVED.read_text(encoding="utf-8"))
...
def saved_get(name: str) -> Dict[str, Any]:
    ...
    q = data.get(name); 
    if q is None: return {"ok": False, "error": "not found"}
    return {"ok": True, "query": q}

def saved_delete(name: str) -> Dict[str, Any]:
    try: data = json.loads(SAVED.read_text(encoding="utf-8"))
    except Exception: data = {}
    if name in data: del data[name]; SAVED.write_text(json.dumps(data, indent=2), encoding="utf-8")
    return {"ok": True}