...
def _load() -> Dict[str, Any]:
    if INDEX_PATH.exists():
        ...
        except Exception: return {"items": [], "t": 0}
    return {"items": [], "t": 0}
...
def _collect() -> List[Dict[str, Any]]:
    ...
    for p in Path("./data/scenes").glob("*.json"):
        try:
            ...
            data = {}
        items.append({
            "type":"scene","key":p.stem,"path":p.as_posix(),"tags":_tags_from_json(data),
            "text": _file_text(p), "size": p.stat().st_size, "updated": p.stat().st_mtime
        })
    # characters
    for p in Path("./data/characters").glob("*.json"):
        ...
        items.append({
            "type":"character","key":p.stem,"path":p.as_posix(),"tags":_tags_from_json(data),
            "text": _file_text(p), "size": p.stat().st_size, "updated": p.stat().st_mtime
        })
    # workflows
    for p in Path("./data/workflows").glob("*.json"):
        items.append({
            "type":"workflow","key":p.stem,"path":p.as_posix(),"tags":[],
            "text": _file_text(p), "size": p.stat().st_size, "updated": p.stat().st_mtime
        })
    # assets
    ad = Path("./data/assets")
    if ad.exists():
        for p in ad.rglob("*"):
            ...
            tags: List[str] = []
            if side.exists():
                try: tags = _tags_from_json(json.loads(side.read_text(encoding="utf-8", errors="replace")))
                except Exception: tags = []
            sha = _sha256(p)
            items.append({
                "type":"asset","key":str(p.relative_to(ad)),"path":p.as_posix(),"tags":tags,
                "text":"", "size": p.stat().st_size, "updated": p.stat().st_mtime, "sha256": sha
            })
    # artifacts (renders)
    rd = Path("./exports/renders")
    if rd.exists():
        for p in rd.rglob("*.png"):
            if "_thumbs" in p.parts: continue
            items.append({
                "type":"artifact","key":str(p.relative_to(rd)),"path":p.as_posix(),"tags":[],
                "text":"", "size": p.stat().st_size, "updated": p.stat().st_mtime
            })
    return items
...
def ensure() -> Dict[str, Any]:
    if not INDEX_PATH.exists(): return reindex()
    try:
        return _load()
    except Exception:
        return reindex()
...
def _score(q: str, it: Dict[str, Any]) -> float:
    ql = q.lower().strip()
    if not ql: return 0.1
    s = (it.get("key","") + " " + it.get("path","") + " " + " ".join(it.get("tags", [])) + " " + (it.get("text","")[:10000])).lower()
    hit = s.count(ql)
    if hit: return min(10.0, 1.0 + hit * 0.5)
    # partials
    if ql in s: return 0.8
    return 0.0

def query(q: str = "", types: Optional[List[str]] = None, tags: Optional[List[str]] = None, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
    db = ensure()
    items = db.get("items", [])
    if types:
    ...
    for it in items:
        sc = _score(q, it)
        if sc > 0 or not q: scored.append((sc, it))
...
def tag_item(kind: str, key: str, add: Optional[List[str]] = None, remove: Optional[List[str]] = None) -> Dict[str, Any]:
    add = add or []; remove = remove or []
    if kind not in SUPPORTED: return {"ok": False, "error": "unsupported"}
    if kind in {"scene","character"}:
        base = Path(f"./data/{kind}s"); p = (base / f"{key}.json").resolve()
        ...
        for t in add: tags.add(str(t))
        for t in remove: tags.discard(str(t))
        data["tags"] = sorted(tags)
        p.write_text(_json.dumps(data, indent=2), encoding="utf-8")
        reindex()
        return {"ok": True, "tags": data["tags"]}
    if kind == "asset":
        base = Path("./data/assets"); p = (base / key).resolve()
        if not p.exists(): return {"ok": False, "error": "not_found"}
        side = p.with_suffix(p.suffix + ".meta.json")
        try:
            data = json.loads(side.read_text(encoding="utf-8", errors="replace")) if side.exists() else {}
        except Exception:
            data = {}
        tags = set([str(x) for x in (data.get("tags") or [])])
        for t in add: tags.add(str(t))
    ...
    try:
        db = json.loads(meta.read_text(encoding="utf-8", errors="replace")) if meta.exists() else {}
    except Exception:
        db = {}
