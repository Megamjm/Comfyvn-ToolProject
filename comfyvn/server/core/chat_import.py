...
TS_NAME_COLON = re.compile(r"^\s*\[?(\d{1,2}:\d{2}(?::\d{2})?)\]?\s*([^:]+?)\s*:\s*(.+)$")
...
def _parse_time_to_epoch(s: str) -> float | None:
    ...
    try:
        ...
        if len(parts) == 2:
            h, m = parts; sec = h*3600 + m*60
        elif len(parts) == 3:
            h, m, s = parts; sec = h*3600 + m*60 + s
        else:
            return None
...
def parse_text(text: str, fmt: str = "auto") -> List[Line]:
    ...
    for raw in text.splitlines():
        ...
        if m:
            ts = _parse_time_to_epoch(m.group(1)); spk = m.group(2).strip(); msg = m.group(3).strip()
        else:
            m2 = BRACKET_NAME_COLON.match(raw)
            if m2:
                ts = None; spk = m2.group(2).strip(); msg = m2.group(3).strip()
            else:
                m3 = NAME_COLON.match(raw)
                if m3:
                    spk = m3.group(1).strip(); msg = m3.group(2).strip()
                else:
...
def apply_alias_map(lines: List[Line], alias: Dict[str, str]) -> List[Line]:
    if not alias: return lines
    norm = {k.strip().lower(): v for k,v in alias.items() if isinstance(k, str) and isinstance(v, str)}
    for ln in lines:
...
def assign_by_patterns(lines: List[Line], rules: List[Dict[str, str]]) -> List[Line]:
    ...
    for r in rules or []:
        try:
            comp.append((re.compile(r.get("pattern",""), re.IGNORECASE), r.get("speaker","")))
        except re.error:
            continue
    for ln in lines:
        for rgx, spk in comp:
            if rgx.search(ln.text):
                ln.speaker = spk; break
    return lines
