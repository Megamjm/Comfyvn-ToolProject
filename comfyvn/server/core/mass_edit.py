...
def _load_scene(scene_id: str) -> Dict[str, Any]:
    ...
    if not p.exists():
        return {"scene_id": scene_id, "title": scene_id, "lines": []}
    try:
...
def _select_indices(
    lines: List[Dict[str, Any]], sel: Dict[str, Any] | None
) -> List[int]:
    if not sel:
    ...
    if "regex" in sel and sel["regex"]:
        pat = re.compile(
            sel["regex"], re.I if sel.get("flags", "").lower().find("i") >= 0 else 0
        )
...
def _apply_op(
    lines: List[Dict[str, Any]], op: Dict[str, Any], ctx: Dict[str, Any]
    ...
    if typ == "regex_replace":
        pat = re.compile(
            op.get("pattern", ""),
            re.I if op.get("flags", "").lower().find("i") >= 0 else 0,
        )
        repl = op.get("repl", "")
        for i in idxs:
        ...
        a = int(op.get("start") or 0)
        cnt = int(op.get("count") or 0)
        to = int(op.get("to") or 0)
        ...
        for i in idxs:
            lines[i]["text"] = val + str(lines[i].get("text", ""))
            changed += 1
    elif typ == "suffix":
        val = str(op.get("value") or "")
        for i in idxs:
            lines[i]["text"] = str(lines[i].get("text", "")) + val
            changed += 1
        ...
        while i < len(lines):
            ...
            while j < len(lines) and lines[j].get("speaker") == cur.get("speaker"):
                cur["text"] = (
                    str(cur.get("text", "")) + sep + str(lines[j].get("text", ""))
                )
                j += 1
                changed += 1
            new.append(cur)
...
def apply_ops(
    ...
    before_lines = [
        f"{l.get('speaker','')}: {l.get('text','')}" for l in (scene.get("lines") or [])
    ]
...
def preview(
    scene_id: str, ops: List[Dict[str, Any]], ctx: Dict[str, Any] | None = None
) -> Dict[str, Any]:
    scene = _load_scene(scene_id)
    before_lines = [
        f"{l.get('speaker','')}: {l.get('text','')}" for l in (scene.get("lines") or [])
