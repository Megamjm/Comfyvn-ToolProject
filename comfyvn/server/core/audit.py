...
from comfyvn.server.core.db import get_db, init_db, ApiTokenRow, UserRow, MembershipRow
...
class AuditLogger:
    ...
    def write(self, rec: Dict[str, Any]):
        ...
        if path.exists():
            try:
                *_, last = path.read_text(encoding="utf-8", errors="ignore").splitlines()
...
class AuditMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ts = time.time()
        # headers
        hdr = {k.lower(): (v if k.lower()!="authorization" else "***") for k,v in request.headers.items()}
        # body (best effort)
        try:
        ...
        try:
            ...
            if legacy and (auth == legacy or auth == f"Bearer {legacy}"):
                actor = {"mode":"legacy", "user":"legacy-admin", "org":"default", "role":"admin"}
                ...
                for db in get_db():
                    ...
                    if tok:
                        ...
                        actor = {"mode":"token","user": u.email if u else tok.user_id, "user_id": tok.user_id, "org": tok.org_id, "role": mem.role if mem else "viewer", "token_id": tok.token_id}
                    break
        except Exception:
            pass
        # call downstream
...
def verify_range(start_ts: float, end_ts: float, secret: str | None = None) -> Dict[str, Any]:
        ...
        if p.exists():
            for line_no, line in enumerate(p.read_text(encoding="utf-8", errors="ignore").splitlines(), start=1):
                try:
                    ...
                    computed = hmac.new(secret.encode("utf-8"), (prev + "|" + json.dumps({k:v for k,v in rec.items() if k!='hash'}, sort_keys=True, separators=(",",":"))).encode("utf-8"), hashlib.sha256).hexdigest() if secret else hashlib.sha256((prev + "|" + json.dumps({k:v for k,v in rec.items() if k!='hash'}, sort_keys=True, separators=(",",":"))).encode("utf-8")).hexdigest()
                    if expected != computed:
                        problems.append({"file": p.name, "line": line_no, "error": "hash mismatch"})
                    prev = expected
