...
from comfyvn.server.core.importers import (export_csv, export_jsonl,
                                           export_markdown, import_csv,
                                           import_discord_json, import_jsonl,
                                           import_markdown, import_slack_json,
                                           import_telegram_json,
                                           lines_to_scene, merge_into_scene)
...
def _load_text_or_path(body: Dict[str, Any]) -> str:
    ...
    if p:
        path = Path(p)
        if not path.exists():
            raise HTTPException(400, "content_path not found")
...
async def speakers_put(
    body: Dict[str, Any], _: bool = Depends(require_scope(["content.write"], cost=5))
):
    if not isinstance(body, dict):
        raise HTTPException(400, "body must be object")
    sd = _load_speaker_dict()
    sd.update({k: str(v) for k, v in (body or {}).items()})
    _save_speaker_dict(sd)
    return {"ok": True}
...
def _parse(source: str, text: str, sd: Dict[str, str]) -> list[dict]:
    ...
    if source == "csv":
        return import_csv(text, sd)
    if source == "jsonl":
        return import_jsonl(text, sd)
    if source == "markdown":
        return import_markdown(text, sd)
    if source == "discord":
        return import_discord_json(text, sd)
    if source == "slack":
        return import_slack_json(text, sd)
    if source == "telegram":
        return import_telegram_json(text, sd)
...
async def do_import(
    source: str,
    body: Dict[str, Any],
    _: bool = Depends(require_scope(["content.write"], cost=5)),
):
    ...
    if project_id:
        ...
        tags.add(f"project:{project_id}")
        sc["tags"] = sorted(list(tags))
    if title:
        sc["title"] = title
...
async def do_export(target: str, scene_id: str):
    ...
    tgt = target.lower()
    if tgt == "jsonl":
        content = export_jsonl(lines)
        ext = "jsonl"
    elif tgt == "markdown":
        content = export_markdown(lines)
        ext = "md"
    elif tgt == "csv":
        content = export_csv(lines)
        ext = "csv"
    else:
