...
def routes(request: Request):
    ...
    for r in request.app.routes:
        try:
            ...
            name = getattr(r, "name", None) or getattr(getattr(r, "endpoint", None), "__name__", "unknown")
...
def checks(base: str | None = None, timeout_s: float = 2.0):
    ...
    return {"ok": overall, "timeout_s": float(timeout_s), "base": base, "components": out}
...
def self_test(request: Request):
    ...
    probes = {
        "system": {"status": 200 if has("/system/health") else 404, "ok": has("/system/health")},
        "render": {"status": 200 if has("/render/health") else 404, "ok": has("/render/health")},
        "scanner": {"status": 200 if has("/scanner/health") else 404, "ok": has("/scanner/health")},
        "jobs": {"status": 200 if has("/jobs/health") else 404, "ok": has("/jobs/health")},
    }
    return {"ok": all(v["ok"] for v in probes.values()), "state": state, "fs": {"ok": True, "error": None}, "probes": probes}