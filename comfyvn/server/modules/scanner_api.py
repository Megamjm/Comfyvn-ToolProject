...
class ScanLogRequest(BaseModel):
    ...
    tail: int = Field(
        0, ge=0, le=10000, description="Number of trailing lines to read if path is set"
    )
    patterns: Optional[List[str]] = Field(
        default=None, description="Simple substring filters to extract"
    )
...
def scan_log(req: ScanLogRequest):
    ...
    errors = sum(
        1 for l in lower if " error" in l or l.startswith("error") or "traceback" in l
    )
    ...
    tail = (
        data[-min(len(data), max(0, req.tail)) :]
        if req.tail
        else (data[-100:] if total > 100 else data)
    )
