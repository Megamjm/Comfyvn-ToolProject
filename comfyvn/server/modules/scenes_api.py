...
def _path(name: str) -> Path: return (SCENE_DIR / f"{name}.json").resolve()
...
async def list_scenes():
    ...
    for p in sorted(SCENE_DIR.glob("*.json")):
        try:
            ...
            data = {}
        out.append({"name": p.stem, "size": p.stat().st_size, "lines": len(data.get("lines", []))})
    return {"ok": True, "items": out}

@router.post("/create")
async def create_scene(body: Dict[str, Any], _: bool = Depends(require_scope(["content.write"]))):
    name = body.get("scene_id") or f"scene_{uuid.uuid4().hex[:8]}"
    rec = {"scene_id": name, "created": time.time(), "lines": body.get("lines") or []}
    p = _path(name); p.write_text(json.dumps(rec, indent=2), encoding="utf-8")
...
async def update_scene(name: str, body: Dict[str, Any], _: bool = Depends(require_scope(["content.write"]))):
    p = _path(name)
    if not p.exists(): raise HTTPException(status_code=404, detail="not found")
    data = json.loads(p.read_text(encoding="utf-8", errors="replace"))
    data.update({k:v for k,v in body.items() if k in {"lines","meta","participants","background","cues"}})
