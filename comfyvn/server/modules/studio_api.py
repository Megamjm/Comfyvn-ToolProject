...
from comfyvn.scene_bundle import (ASSETS_MANIFEST_DEFAULT, SCHEMA_PATH_DEFAULT,
                                  build_bundle)
...
async def export_bundle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if raw_path and not raw:
        ...
        if raw is None:
            raise HTTPException(
                status_code=400, detail="raw_path does not exist or is invalid"
            )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"bundle export failed: {exc}"
        ) from exc
    ...
    if out_path_value:
        ...
        logger.info(
            "Bundle generated (not written to disk) for project %s", STATE["project_id"]
        )
