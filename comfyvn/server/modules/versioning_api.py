...
async def list_snaps():
    items = []
    for p in sorted(
        VDIR.glob("snap_*.zip"), key=lambda x: x.stat().st_mtime, reverse=True
    ):
...
async def snapshot(
    ...
    _: bool = Depends(require_scope(["projects.write"])),
):
...
async def download(file: str):
    ...
    if not str(fp).startswith(str(VDIR)):
