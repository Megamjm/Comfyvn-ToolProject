...
from comfyvn.server.core.db import (CharacterRow, RunRow, SceneRow, TokenRow,
                                    as_json_str, from_json_str, get_db,
                                    init_db, is_enabled)
...
async def scenes_upsert(
    ...
    if r:
        r.data = as_json_str(body)
        r.tags = tags
        r.updated = time.time()
    else:
        r = SceneRow(
            scene_id=scene_id,
            data=as_json_str(body),
            tags=tags,
            created=time.time(),
            updated=time.time(),
        )
...
async def characters_list(db: Session = Depends(get_db)):
    ...
    return {
        "ok": True,
        "items": [
            {"name": r.name, "tags": json.loads(r.tags or "[]"), "updated": r.updated}
            for r in rows
        ],
    }
...
async def sync_files_to_db(
    db: Session = Depends(get_db), _: bool = Depends(require_scope(["content.write"]))
):
    # scenes
...
async def sync_db_to_files(
    ...
    for r in db.query(SceneRow).all():
        try:
            ...
            (sdir / f"{r.scene_id}.json").write_text(
