...
from comfyvn.server.core.search_index import reindex, search, facets, saved_list, saved_put, saved_get, saved_delete, INDEX_DIR, MANIFEST
...
async def status():
    ...
    try:
        ...
        return {"ok": True, "index_dir": INDEX_DIR.as_posix(), "count": total, "mtime": (INDEX_DIR.stat().st_mtime if INDEX_DIR.exists() else 0)}
    except Exception as e:
        return {"ok": False, "error": str(e)}

@router.post("/reindex")
async def rebuild(body: Dict[str, Any] = None, _: bool = Depends(require_scope(["search.write"], cost=5))):
    body = body or {}
...
async def saved_get_ep(name: str):
    r = saved_get(name)
    if not r.get("ok"): raise HTTPException(status_code=404, detail="not found")
    return r
