...
from comfyvn.server.core.artifacts import get_local, list_local, register
from comfyvn.server.core.db import (ArtifactRow, as_json_str, from_json_str,
                                    get_db, init_db, is_enabled)
...
async def put(
    body: Dict[str, Any],
    _: bool = Depends(require_scope(["artifacts.write"])),
    ...
    if not j.get("ok"):
        raise HTTPException(status_code=400, detail=j.get("error", "error"))
    item = j["item"]
    if is_enabled():
        init_db()
        try:
...
async def list_items(
    ...
    if is_enabled():
        ...
        try:
            ...
            if run_id:
                q = q.filter(ArtifactRow.run_id == run_id)
            if scene_id:
                q = q.filter(ArtifactRow.scene_id == scene_id)
            if kind:
                q = q.filter(ArtifactRow.kind == kind)
