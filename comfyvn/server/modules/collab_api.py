...
async def ws(websocket: WebSocket):
    ...
    try:
        await websocket.send_text(json.dumps({"type":"presence","scene_id":scene_id,"data":HUB.presence(scene_id)}))
        while True:
            msg = await websocket.receive_text()
            try: body = json.loads(msg)
            except Exception: body = {"type":"ping"}
            typ = body.get("type")
            if typ == "ping":
                await websocket.send_text(json.dumps({"type":"pong"}))
            elif typ == "presence":
                await websocket.send_text(json.dumps({"type":"presence","data": HUB.presence(scene_id)}))
            elif typ == "lock.acquire":
                ok = HUB.acquire_lock(scene_id, cid, body.get("ttl"))
                await websocket.send_text(json.dumps({"type":"lock","ok": ok, "data": HUB.presence(scene_id)}))
                ...
                try:
                    sc2 = dict(sc); sc2["scene_id"]=scene_id
                    if body.get("title") is not None: sc2["title"] = body.get("title")
                    if isinstance(body.get("lines"), list): sc2["lines"] = body.get("lines")
                    scene_save(sc2, expected_version=base)
                    payload = {"type":"applied","scene_id":scene_id,"version": sc2.get("version"), "scene": sc2}
                    for uid,c in list(HUB.rooms.get(scene_id, {}).items()):
                        try: await c.ws.send_text(json.dumps(payload))
                        except Exception: pass
                except Exception as e:
                    await websocket.send_text(json.dumps({"type":"conflict","scene_id":scene_id,"error":str(e),"latest": sc}))
            else:
        ...
        try:
            payload = {"type":"presence", "scene_id": scene_id, "data": HUB.presence(scene_id)}
            for uid,c in list(HUB.rooms.get(scene_id, {}).items()):
