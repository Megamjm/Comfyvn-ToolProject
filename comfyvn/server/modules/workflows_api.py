...
router = APIRouter()
WF_DIR = Path("data/workflows"); WF_DIR.mkdir(parents=True, exist_ok=True)
SCENE_DIR = Path("data/scenes"); SCENE_DIR.mkdir(parents=True, exist_ok=True)
EXPORTS = Path("exports/renpy"); EXPORTS.mkdir(parents=True, exist_ok=True)
...
def _ok(data=None): return {"ok": True, **(data or {})}
def _wf_path(name: str) -> Path: return WF_DIR / f"{name}.json"
def _scene_path(scene_id: str) -> Path: return SCENE_DIR / f"{scene_id}.json"
...
def validate_workflow(payload: dict = Body(...)):
    ...
    if not required.issubset(keys):
        return _ok({"validated": False, "missing": sorted(required - keys), "payload_keys": list(keys)})
...
def _run_exporter() -> tuple[bool, str]:
    try:
        ...
        p = subprocess.run([exe, "scripts/export_renpy.py", "--quiet"], capture_output=True, text=True, check=True)
...
def instantiate_workflow(payload: dict = Body(...)):
    ...
    (WF_DIR / f"done_{jid}.json").write_text(json.dumps({
        "job_id": jid, "workflow": payload.get("name") or "draft",
        "scene_file": str(sp), "export_ok": ok, "export_msg": msg, "export_dir": str(EXPORTS)
    }, indent=2), encoding="utf-8")
    return _ok({"job_id": jid, "workflow": payload.get("name") or "draft",
                "scene_file": str(sp), "export_ok": ok, "export_msg": msg, "export_dir": str(EXPORTS)})
