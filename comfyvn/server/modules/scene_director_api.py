...
from comfyvn.core.memory_engine import Personas, Lore, Voices, remember_event
...
def compose(payload: dict = Body(...)):
    ...
    if not chars: raise HTTPException(400, "characters required")
    ...
    for c in chars:
        ...
        lines.append({"speaker": c, "text": f"{c} ({trait}, {mood}): reacts to '{context}'"})
    remember_event("director.compose", {"chars": chars, "world": world})
    return {"ok": True, "world": world, "lore_refs": len(lore.get('entries',[])), "lines": lines}