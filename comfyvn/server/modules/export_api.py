...
def _read_json(path: Path, *, entity: str) -> dict:
    if not path.exists():
        raise HTTPException(status_code=404, detail=f"{entity} not found: {path.as_posix()}")
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise HTTPException(status_code=400, detail=f"{entity} invalid JSON: {path}") from exc
...
def _candidate_timeline_paths(timeline_id: str, project_id: Optional[str]) -> Iterable[Path]:
    seen: set[Path] = set()
...
def _iter_scene_dialogue(scene: dict) -> Iterable[dict]:
    ...
    yield {"type": "line", "speaker": None, "text": scene.get("title") or scene.get("id") or ""}
...
def _build_renpy_project(
    *,
    ...
    if not scene_ids:
        raise HTTPException(status_code=400, detail="timeline contains no scenes to export")
...
def _load_character(character_id: str, project_id: Optional[str]) -> tuple[Optional[dict], Optional[Path]]:
    candidates = []
    if project_id:
        candidates.append(data_dir("projects", project_id, "characters") / f"{character_id}.json")
...
def _ensure_timeline_payload(
    ...
    candidate_id = timeline_id or project_data.get("timeline_id") or f"{project_id}_timeline"
    try:
        ...
        timeline_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
...
def _build_bundle_archive(
    *,
    ...
    for scene_id in renpy_info["scene_ids"]:
        scene, path = _load_scene(scene_id, project_id)
        scenes[scene_id] = scene
        scene_sources[scene_id] = path
    ...
    with zipfile.ZipFile(bundle_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr("provenance.json", json.dumps(provenance, indent=2, ensure_ascii=False))
...
def export_renpy(
    ...
    project_id: Optional[str] = Query(None, description="Optional project context for scene lookups"),
):
    project_data: Optional[dict] = None
    if project_id:
        project_data, _ = _load_project(project_id)

    timeline, _ = _load_timeline(timeline_id, project_id) if timeline_id else ({}, Path())
...
def export_bundle(
    ...
    timeline_id: Optional[str] = Query(None, description="Optional timeline identifier to prioritise"),
):
    project_data, _ = _load_project(project_id)
