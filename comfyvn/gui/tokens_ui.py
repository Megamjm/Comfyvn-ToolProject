...
class TokensUI(QWidget):
    def __init__(self, parent=None):
        ...
        b_add = QPushButton("Create")
        b_add.clicked.connect(self.create)
        b_del = QPushButton("Revoke")
        b_del.clicked.connect(self.revoke)
        form.addWidget(QLabel("Name"))
        form.addWidget(self.name)
        form.addWidget(QLabel("Scopes(csv)"))
        form.addWidget(self.scopes)
        form.addWidget(b_add)
        form.addWidget(b_del)
        lay = QVBoxLayout(self)
        lay.addWidget(QLabel("User Tokens"))
        lay.addLayout(form)
    ...
    def refresh(self):
        ...
        for t in self.bridge.tokens_list():
            it = QListWidgetItem(
                f"{t.get('id')}  {t.get('name')}  scopes={','.join(t.get('scopes',[]))}"
            )
    ...
    def create(self):
        ...
        QMessageBox.information(
            self, "Token", f"Created: {out.get('token',{}).get('id')}"
        )
        self.refresh()
