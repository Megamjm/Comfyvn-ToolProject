...
class SnapshotDefaultsDrawer(QWidget):
    def __init__(self, parent=None, api_base="http://127.0.0.1:8001"):
        ...
        self.combo_snaps = QComboBox(); v.addWidget(self.combo_snaps)
        ...
        for b in (self.btn_refresh, self.btn_create, self.btn_restore): row.addWidget(b)
        v.addLayout(row)
    ...
    def _restore_snapshot(self):
        ...
        if not name:
            ...
            return
        if QMessageBox.question(self, "Confirm", f"Restore '{name}'? This overwrites data/exports.", QMessageBox.Yes | QMessageBox.No) != QMessageBox.Yes:
            return
