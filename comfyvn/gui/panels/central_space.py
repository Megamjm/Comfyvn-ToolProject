...
class CentralSpace(QWidget):
    ...
    def __init__(self, open_assets=None, open_timeline=None, open_logs=None):
        ...
        row = QHBoxLayout()
        b1 = QPushButton("Open Assets")
        b1.clicked.connect(open_assets or (lambda: None))
        b2 = QPushButton("Open Timeline")
        b2.clicked.connect(open_timeline or (lambda: None))
        b3 = QPushButton("Show Logs")
        b3.clicked.connect(open_logs or (lambda: None))
        row.addWidget(b1)
        row.addWidget(b2)
        row.addWidget(b3)
        row.addStretch(1)
