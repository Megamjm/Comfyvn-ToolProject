...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QListWidget, QLabel, QListWidgetItem
...
class _PollWorker(QObject):
    result = Signal(dict)
    def __init__(self, base, jid): super().__init__(); self.base=base; self.jid=jid
    def run(self):
        try:
...
class RenderPanel(QWidget):
    ...
    def __init__(self, base: str = "http://127.0.0.1:8001"):
        ...
        hb.addWidget(self.btn_submit); hb.addWidget(self.btn_refresh); hb.addStretch(1)
        v.addLayout(hb)

        self.list = QListWidget(self); v.addWidget(self.list,1)
        self.lbl = QLabel("", self); v.addWidget(self.lbl)

        self.btn_submit.clicked.connect(self._submit)
        self.btn_refresh.clicked.connect(self._refresh)

        self.timer = QTimer(self); self.timer.setInterval(1200); self.timer.timeout.connect(self._poll_all)
        self.timer.start()

    def _submit(self):
        res = self.service.submit_dummy()
        if not res.get("ok"):
            self.lbl.setText(f"Submit failed: {res}"); return
        jid = res.get("id")
        it = QListWidgetItem(f"{jid} — queued (0%)"); it.setData(Qt.UserRole, jid)
        self.list.addItem(it)
        self.lbl.setText(f"Submitted {jid}")

    def _refresh(self): self._poll_all()
    ...
    def _poll_one(self, item: QListWidgetItem, jid: str):
        th = QThread(self)
        wk = _PollWorker(self.base, jid); wk.moveToThread(th)
        wk.result.connect(lambda payload: self._on_result(item, payload))
        th.started.connect(wk.run); th.start()

    def _on_result(self, item: QListWidgetItem, payload: dict):
        d = payload.get("data",{})
        if not d.get("ok"):
            item.setText(f"{payload.get('jid')} — error"); return
        job = d.get("job",{})
