...
from PySide6.QtWidgets import (QDockWidget, QFileDialog, QGraphicsScene,
                               QGraphicsView, QMessageBox, QVBoxLayout,
                               QWidget)
    ...
    def open_scene_json(self, parent=None):
        try:
            p = QFileDialog.getOpenFileName(
                parent, "Open Scene JSON", str(DATA_SCENE.parent), "JSON Files (*.json)"
            )[0]
            if p:
