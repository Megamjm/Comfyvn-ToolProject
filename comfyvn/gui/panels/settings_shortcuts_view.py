...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QTableWidget, QTableWidgetItem, QHeaderView, QInputDialog, QMessageBox, QPushButton, QHBoxLayout
...
class SettingsShortcutsView(QWidget):
    def __init__(self):
        ...
        hb.addWidget(self.btn_rebind); hb.addWidget(self.btn_clear); hb.addStretch(1)
        lay.addLayout(hb)
    ...
    def _rebinder(self, *args):
        ...
        if not name:
            QMessageBox.information(self, "Shortcuts", "Select an action first."); return
        current = ShortcutManager.list_shortcuts().get(name, "")
        new_seq, ok = QInputDialog.getText(self, "Rebind Shortcut", f"{name}\nCurrent: {current}\nNew:")
        if ok:
