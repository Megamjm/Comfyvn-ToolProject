...
class ServerControlPanel(QWidget):
    def __init__(self, api_base="http://127.0.0.1:8001", parent=None):
        ...
        for L in (self.lbl_server, self.lbl_cpu, self.lbl_ram, self.lbl_gpu, self.lbl_tasks):
    ...
    def _tick(self):
        # server status
        health = _fetch_json(self.api_base + "/health") or _fetch_json(self.api_base + "/healthz")
        ...
        gpu_txt = "GPU: -"
        if m:
            if "cpu" in m: cpu_txt = f"CPU: {m['cpu']}%"
            if "mem" in m: ram_txt = f"RAM: {m['mem']}%"
            if m.get("gpus"):
                g = m["gpus"]
                if isinstance(g, list) and g:
                    parts = []
                    for gg in g[:2]:
                        parts.append(f"#{gg.get('id','?')} {gg.get('util','?')}% {gg.get('temp_c','?')}°C")
                    if len(g) > 2:
        ...
        if isinstance(t, dict):
            ...
            pass
        self.lbl_tasks.setText("Tasks: server on standby" if tasks_num == 0 else f"Tasks: {tasks_num} running")