...
class TaskManagerDock(QWidget):
    ...
    def _ensure_row(self, tid, name):
        if tid in self.rows: return self.rows[tid]
    ...
    def _update_prog(self, tid, pct):
        ...
        if isinstance(w,QProgressBar): w.setValue(int(pct))

    def _on_enq(self, d):  self._ensure_row(d["id"], d["name"])
    def _on_start(self, d): self._update_status(d["id"], "running")
    def _on_prog(self, d):  self._update_prog(d["id"], d["progress"])
    def _on_done(self, d):  self._update_status(d["id"], "done"); self._update_prog(d["id"], 100)
    def _on_err(self, d):   self._update_status(d["id"], "error")