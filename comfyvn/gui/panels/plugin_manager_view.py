...
from comfyvn.core.extension_loader import discover_extensions, reload_extensions, set_enabled
...
class PluginManagerView(QWidget):
    def __init__(self):
        ...
        self.btn_enable = QPushButton("Enable"); self.btn_disable = QPushButton("Disable")
        self.btn_reload = QPushButton("Reload All")
        hb.addWidget(self.btn_enable); hb.addWidget(self.btn_disable); hb.addStretch(1); hb.addWidget(self.btn_reload)
    ...
    def refresh(self):
        ...
        st = {}
        try:
            with open(self._state_file(),"r",encoding="utf-8") as f: st = json.load(f)
        except Exception: st = {}
        if os.path.exists(base):
            for root, dirs, files in os.walk(base):
                if "plugin_manifest.json" in files:
                    ...
                    try:
                        with open(path,"r",encoding="utf-8") as f: m = json.load(f)
