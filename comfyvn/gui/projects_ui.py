...
class ProjectsUI(QWidget):
    def __init__(self, parent=None):
        ...
        b_sel.clicked.connect(self.select)
        form.addWidget(QLabel("Name"))
        form.addWidget(self.name)
        form.addWidget(b_add)
        form.addWidget(b_sel)
    ...
    def refresh(self):
        ...
        for p in self.bridge.projects():
            it = QListWidgetItem(
                f"{p.get('name')}  scenes={p.get('scenes')} chars={p.get('characters')} {'[current]' if p.get('current') else ''}"
            )
    ...
    def select(self):
        ...
        self.bridge.projects_select(name)
        self.refresh()
