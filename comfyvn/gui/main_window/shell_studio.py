...
class ShellStudio(QMainWindow):
    def __init__(self, title="ComfyVN Studio"):
        ...
        self.mb = QMenuBar(self); self.setMenuBar(self.mb)
        self.sb = QStatusBar(self); self.setStatusBar(self.sb)
        ...
        a_exit = QAction("Exit", self); a_exit.triggered.connect(self.close)
        self.m_file.addAction(a_exit)

    def add_view_item(self, text, fn):
        a = QAction(text, self); a.triggered.connect(fn); self.m_view.addAction(a); return a

    def add_tools_item(self, text, fn):
        a = QAction(text, self); a.triggered.connect(fn); self.m_tools.addAction(a); return a