...
class ArtifactsBrowser(QWidget):
    def __init__(self, parent=None):
        ...
        bar = QHBoxLayout()
        bar.addWidget(QLabel("Artifacts"))
        bar.addWidget(btn)
    ...
    def refresh(self):
        ...
        for it in self.bridge.artifacts():
            q = QListWidgetItem(f"{it.get('path')}  ({it.get('size')})")
            q.setData(Qt.UserRole, it.get("path"))
            self.list.addItem(q)
    ...
    def _on_select(self, cur, prev):
        ...
        try:
            ...
            if pm.isNull():
                ...
                self.preview.setPixmap(pm.scaledToHeight(320, Qt.SmoothTransformation))
