...
from PySide6.QtWidgets import (QDialog, QFileDialog, QFormLayout, QLabel,
...
class ImportCenterWindow(QDialog):
    ...
    def _tab_generic(self, label: str, default_dir: str):
        ...
        f = QFormLayout(w)
        self._src = None
        self._dst = Path(default_dir)
        lbl_src = QLabel("—", w)
        lbl_dst = QLabel(str(self._dst.resolve()), w)
        ...
        def pick_src():
            fn, _ = QFileDialog.getOpenFileName(
                self, f"Choose {label} Zip", "", "Zip (*.zip)"
            )
            if fn:
                self._src = Path(fn)
                lbl_src.setText(fn)
        ...
        btn_dst.clicked.connect(pick_dst)
        btn_go.clicked.connect(do_import)
        f.addRow("Source:", lbl_src)
        f.addRow(btn_src)
        f.addRow("Output:", lbl_dst)
        f.addRow(btn_dst)
        f.addRow(btn_go)
        return w
    ...
    def _extract(self, src: Path | None, dst: Path):
        if not src:
            raise RuntimeError("No source chosen")
