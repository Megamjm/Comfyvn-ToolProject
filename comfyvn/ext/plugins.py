...
class PluginManager:
    ...
    def register(self, plugin: Plugin, user: bool = False):
        ...
        if user: self._user_plugin_names.add(plugin.name)
        for t, fn in (plugin.jobs or {}).items():
            self.job_handlers[t] = fn
        if plugin.on_event: self.event_handlers.append(plugin.on_event)
    ...
    def handle(self, typ: str, payload: dict, job_id: Optional[str]):
        ...
        if not fn: return {"ok": False, "error": f"no handler for {typ}"}
        try: return fn(payload, job_id)
    ...
    def _load_file(self, p: Path, user: bool):
        try:
            ...
            if isinstance(plug, Plugin): self.register(plug, user=user)
        except Exception:
            pass

    def reload_from_dir(self, path: str = "./data/plugins"):
        self.plugins = [p for p in self.plugins if p.name not in self._user_plugin_names]
        self.job_handlers = {t:fn for t,fn in self.job_handlers.items() if t in {}}  # cleared
