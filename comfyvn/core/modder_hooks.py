...
HOOK_SPECS: Dict[str, HookSpec] = {
    ...
    "on_audio_tts_cached": HookSpec(
        name="on_audio_tts_cached",
        description="Emitted after the TTS cache writes or reuses a synthesized clip.",
        payload_fields={
            "cache_key": "Deterministic cache key derived from (character,text,style,model).",
            "path": "Absolute path to the cached WAV artifact.",
            "sidecar": "Absolute path to the generated sidecar JSON.",
            "character": "Character label submitted with the request.",
            "style": "Declared speaking style for the clip.",
            "model": "Identifier of the synthesis backend/model.",
            "seed": "Optional numeric seed provided for downstream DSP.",
            "voice_id": "Voice preset identifier when a catalog match was found.",
            "text_sha1": "SHA-1 digest of the submitted text payload.",
            "text_length": "Number of characters in the submitted text.",
            "checksum_sha1": "SHA-1 digest of the rendered WAV samples.",
            "bytes": "Size of the WAV artifact in bytes when available.",
            "cached": "Boolean flag indicating whether the clip already existed.",
            "provenance": "Provenance payload recorded alongside the sidecar.",
        },
        ws_topic="audio.tts.cached",
        rest_event="on_audio_tts_cached",
    ),
    "on_audio_alignment_generated": HookSpec(
        name="on_audio_alignment_generated",
        description="Raised whenever phoneme alignment JSON is generated for a clip.",
        payload_fields={
            "text_sha1": "SHA-1 digest of the input text that produced the alignment.",
            "alignment": "List of phoneme alignment entries ({phoneme,t_start,t_end}).",
            "alignment_path": "Filesystem path where the alignment JSON was written.",
            "lipsync_path": "Optional filesystem path to a derived lipsync payload.",
            "fps": "Frame rate used for the generated lipsync payload (if any).",
            "character": "Character label provided with the alignment request.",
            "style": "Style label provided with the alignment request.",
            "model": "Model label provided with the alignment request.",
            "alignment_checksum": "Deterministic checksum recorded alongside the alignment payload.",
        },
        ws_topic="audio.alignment.generated",
        rest_event="on_audio_alignment_generated",
    ),
    "on_audio_mix_rendered": HookSpec(
        name="on_audio_mix_rendered",
        description="Broadcast when the audio mixer writes or reuses a cached mix.",
        payload_fields={
            "cache_key": "Deterministic cache key derived from tracks/ducking/sample_rate.",
            "path": "Absolute path to the cached mix WAV artifact.",
            "sidecar": "Absolute path to the mix metadata JSON.",
            "duration": "Duration of the rendered mix in seconds.",
            "sample_rate": "Sample rate used for rendering the mix.",
            "tracks": "Track metadata (path, role, gain, offset) resolved during the render.",
            "ducking": "Ducking configuration applied, when provided.",
            "checksum_sha1": "SHA-1 digest of the rendered WAV samples.",
            "bytes": "Size of the WAV artifact in bytes when available.",
            "cached": "Boolean flag indicating whether the mix already existed.",
            "rendered_at": "ISO8601 timestamp for when the mix metadata was last updated.",
        },
        ws_topic="audio.mix.rendered",
        rest_event="on_audio_mix_rendered",
    ),
    "on_prop_applied": HookSpec(
        ...
        payload_fields={
            ...
            "anchor": "Anchor payload containing id/x/y coordinates.",
            "z_order": "Layer bucket for the prop (background/midground/foreground/ui).",
            "visible": "Boolean result after evaluating the supplied conditions.",
            "tween": "Tween payload describing duration/ease/hold/stop_at_end/loop.",
            ...
            "context": "Flattened evaluation context derived from the caller state.",
    ...
    "on_weather_changed": HookSpec(
        name="on_weather_changed",
        description="Emitted when the weather planner compiles a new overlay/light plan.",
        payload_fields={
            ...
            "sfx": "Loop path, gain, fade-in/out, and tags applied to the ambience layer.",
            "lut": "Color lookup data (path/tint/intensity) for exporters.",
            "bake_ready": "Boolean flag toggled when background layers can be baked.",
            "flags": "Plan flags (e.g. bake_background) for downstream automation.",
        ...
        ws_topic="modder.on_weather_changed",
        rest_event="on_weather_changed",
    ),
    "on_accessibility_settings": HookSpec(
        ...
        payload_fields={
            "state": "Persisted accessibility settings (font_scale, color_filter, high_contrast, subtitles_enabled, ui_scale, view_overrides).",
            "timestamp": "Event emission timestamp (UTC seconds).",
    ...
    "on_accessibility_input_map": HookSpec(
        ...
        payload_fields={
            ...
            "reason": "Change cause (update, reset, import).",
        },
    ...
    "on_battle_simulated": HookSpec(
        ...
        payload_fields={
            ...
            "breakdown": "List of roll breakdown entries with component totals.",
            "formula": "Formula label describing the deterministic roll (e.g. v0).",
    ...
    "on_battle_resolved": HookSpec(
        ...
        payload_fields={
            ...
            "editor_prompt": "Prompt label surfaced to editors when selecting an outcome.",
            "formula": "Formula label used to produce optional narration/log payloads.",
            "seed": "Seed used when narration log entries are generated.",
            "narration": "Single-line narration preview when stats were supplied.",
            "log": "Turn-by-turn narration entries generated for the resolve call.",
            "timestamp": "Event emission timestamp (UTC seconds).",
        },
