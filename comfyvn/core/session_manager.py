...
def load() -> Dict[str, Any]:
    _ensure()
    if not os.path.exists(FILE): return DEFAULT.copy()
    try:
...
def remember_space(space: str):
    s = load(); s["last_space"] = space; save(s)

def remember_panel_open(title: str):
    s = load(); p = set(s.get("open_panels", [])); p.add(title); s["open_panels"] = sorted(p); save(s)

def remember_panel_closed(title: str):
    s = load(); p = [x for x in s.get("open_panels", []) if x != title]; s["open_panels"] = p; save(s)