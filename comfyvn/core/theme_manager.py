...
def _qss_for(pal: dict) -> str:
    ...
    qss = []
    qss.append(f"QWidget {{ background: {bg}; color: {tx}; selection-background-color: {acc}; }}")
    qss.append("QMenuBar, QMenu, QToolBar, QDockWidget, QStatusBar, QTabBar, "
               f"QTabWidget, QTreeView, QListView, QTextEdit, QPlainTextEdit {{ background: {pbg}; color: {tx}; border: 1px solid {bd}; }}")
    qss.append(f"QPushButton {{ background: {bd}; border: 1px solid {bd}; padding: 6px 10px; border-radius: 6px; }}")
