...
import redis
from comfyvn.server.core.redis_support import STREAM, EVENTS, GROUP, _r
...
def _run_job(r: redis.Redis, jid: str):
    ...
    if rec.get("status") == "canceled":
        _emit(r, "job:canceled", {"id": jid, "phase":"pre"}); return
    if not _deps_done(r, rec):
        time.sleep(0.5); r.xadd(STREAM, {"id": jid}); return

    typ = rec.get("type"); payload = json.loads(rec.get("payload","{}"))
    ...
    plugins = PluginManager(); plugins.register(echo_plugin); plugins.register(webhook_plugin)

    def on_event(k, d): _emit(r, k, d)
    ...
    try:
        if typ == "render":
            ...
            results = []; n = max(1, len(items))
            for i, it in enumerate(items):
                _emit(r, "job:progress", {"id": jid, "p": i/n})
                rid = it.get("id") or f"{jid[:8]}_{i+1}"; it = {**it, "id": rid}
                res = pm.render_character_with_progress(it, jid); results.append(res)
        ...
        if out.get("ok"):
            ...
            if attempt <= retries:
                r.hset(key, mapping={"status":"queued","error": out.get("error","unknown")})
                r.xadd(STREAM, {"id": jid})
                _emit(r, "job:requeued", {"id": jid, "reason":"retry"})
            else:
                r.hset(key, mapping={"status":"error","error": out.get("error","unknown")})
...
def main():
    ...
    while True:
        ...
        if time.time() - last_idle_check > 5.0:
            try:
                pend = r.xpending_range(STREAM, GROUP, min="-", max="+", count=50, consumername=None, idle=int(VISIBILITY_TIMEOUT*1000))
                for p in pend:
                    try: r.xclaim(STREAM, GROUP, CONSUMER, min_idle_time=int(VISIBILITY_TIMEOUT*1000), message_ids=[p['message_id']])
        ...
        try:
            ...
            if "NOGROUP" in str(e):
                r.xgroup_create(STREAM, GROUP, id="$", mkstream=True); continue
            resp = []
