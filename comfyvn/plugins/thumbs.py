...
TH_DIR = Path("./data/assets/_thumbs"); TH_DIR.mkdir(parents=True, exist_ok=True)

def make_thumb(payload: Dict[str, Any], job_id: str | None):
    path = payload.get("path"); size = int(payload.get("size") or 256)
    ...
    try:
        with Image.open(p) as im:
            ...
            bg = Image.new("RGB", im.size, (0,0,0)); bg.paste(im, (0,0))
