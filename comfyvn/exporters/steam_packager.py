...
LOGGER = get_logger("export.publish.steam", component="export.publish", target="steam")
...
def package(export_result: ExportResult, options: PackageOptions) -> PackageResult:
    ...
    target_root = ensure_publish_root(publish_root / "steam")
    label = (
        options.label
        or export_result.manifest_payload.get("project", {}).get("title")
        or export_result.project_id
    )
    slug = package_slug(export_result.project_id, label, options.version, "steam")
    archive_path = target_root / f"{slug}.steam.zip"
    manifest_path = target_root / f"{slug}.steam.manifest.json"
    license_manifest_path = target_root / f"{slug}.steam.licenses.json"
    hooks_path = target_root / f"{slug}.steam.hooks.json"
    ...
    builder.add_bytes(f"{slug}/steam/assets/icon.png", icon_bytes)

    eula_text = resolve_eula_text(options, target="steam", label=label)
    builder.add_bytes(f"{slug}/steam/legal/EULA.txt", eula_text.encode("utf-8"))

    license_text = resolve_license_text(options, license_manifest)
    builder.add_bytes(f"{slug}/steam/legal/LICENSE.txt", license_text.encode("utf-8"))

    license_manifest_json = json.dumps(license_manifest, indent=2, ensure_ascii=False)
    builder.add_bytes(
        f"{slug}/steam/legal/license_manifest.json",
        license_manifest_json.encode("utf-8"),
    )

    package_manifest_json = json.dumps(package_manifest, indent=2, ensure_ascii=False)
    builder.add_bytes(
        f"{slug}/steam/publish_manifest.json", package_manifest_json.encode("utf-8")
    )
    ...
    for platform in platforms:
        for rel, source in files:
            arcname = f"{slug}/steam/builds/{platform}/game/{rel}"
    ...
    if options.include_debug:
        ...
        builder.add_bytes(
            f"{slug}/steam/debug/modder_hooks.json", hooks_json.encode("utf-8")
        )
    ...
    manifest_stamp = provenance.stamp_path(
        manifest_path,
        source="export.publish.steam.manifest",
        inputs={
            "target": "steam",
            "label": label,
            "version": options.version,
