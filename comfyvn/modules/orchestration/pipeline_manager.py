...
class PipelineOptions:
    ...
    output_file: str = "latest.png"     # ComfyUI output file name (saved under its configured dir)
    export_dir: str = "./exports/renpy" # Ren'Py export dir (RenPyBridge should also ensure)
...
class PipelineManager:
    ...
    # ---------- Utilities ----------
    def _emit(self, job_id: str, status: str, progress: float, meta: Optional[Dict[str, Any]] = None):
        """Convenience wrapper to update JobManager and keep progress consistent."""
        payload: Dict[str, Any] = {"status": status, "progress": max(0.0, min(1.0, float(progress)))}
        if meta:
            payload.update(meta)
    ...
    # ---------- Core steps ----------
    def _lm_generate(self, scene_input: Dict[str, Any], options: PipelineOptions) -> Dict[str, Any]:
        """
        ...
        """
        messages = [
            {"role": "system", "content": "You are a visual novel scene writer. Expand and polish dialogue succinctly."},
            {"role": "user", "content": json.dumps(scene_input, ensure_ascii=False)}
        ]
        res = self.ctx.lm_bridge.chat(messages, model=options.lm_model, max_tokens=options.lm_max_tokens)
        ...
        return {"text": scene_input.get("text", ""), "lm_error": res.get("error", "unknown_error") if res else "no_response"}

    def _render(self, prompt_text: str, options: PipelineOptions, progress_cb: Optional[Callable[[float, str], None]] = None) -> Dict[str, Any]:
        ...
        try:
            result = self.ctx.comfy_bridge.queue_and_wait(prompt_text, output_file=options.output_file, wait=options.wait_render, progress_cb=_cb)
        except TypeError:
            # Older comfy_bridge without progress_cb param:
            result = self.ctx.comfy_bridge.queue_and_wait(prompt_text, output_file=options.output_file, wait=options.wait_render)

        # If waiting synchronously wasn't supported by bridge, we can poll here as a fallback:
        if options.wait_render and isinstance(result, dict) and result.get("status") == "polling":
            job_id = result.get("job_id")
            start = time.time()
            while time.time() - start < options.render_timeout:
                hist = self.ctx.comfy_bridge.poll_job(job_id, timeout=int(options.render_interval), interval=options.render_interval)
                if hist.get("status") == "complete":
                    return hist
                if progress_cb:
                    progress_cb(0.6 + 0.3 * ((time.time() - start) / max(1, options.render_timeout)), "waiting")
    ...
    def run_scene_pipeline(
        ...
        input_path = self._archive_json(scene_input, scene_input.get("scene_id", "scene_input"), opts.archive_dir)
        ...
        if opts.use_lm:
            ...
            scene_input = {**scene_input, **({"text": lm_out.get("text", scene_input.get("text", ""))})}
            lm_meta_path = self._archive_json(lm_out, scene_input.get("scene_id", "scene_input") + "_lm", opts.archive_dir)
        else:
        ...
        plan_path = self._archive_json(plan, plan.get("scene_id", "scene_plan"), opts.archive_dir)
        ...
        if opts.do_render:
            ...
            def progress_cb(pct: float, stage: str):
                ...
                self._emit(job_id, f"processing:render_{stage}", mapped, {"stage": "render"})

            render_result = self._render(plan["render_ready_prompt"], opts, progress_cb=progress_cb)
            self._emit(job_id, "processing:render_done", 0.88, {"stage": "render", "render_status": render_result.get("status") if isinstance(render_result, dict) else "unknown"})
        ...
        self._emit(job_id, "complete", 1.0, {"stage": "done", "result": {"has_render": bool(render_result), "has_script": bool(renpy_result)}})
    ...
    def run_chapter_pipeline(
        ...
        for idx, raw_scene in enumerate(scenes, start=1):
            ...
            self._emit(job_id, f"processing:scene_{idx}/{total}", step_base, {"stage": "scene", "scene_index": idx})
            ...
            arc = self._archive_json(plan, plan.get("scene_id", f"scene_{idx}"), opts.archive_dir)
            ...
            if opts.do_render:
                def cb(pct: float, stage: str):
                    ...
                    self._emit(job_id, f"processing:scene_{idx}_render_{stage}", min(mapped, 0.98), {"stage": "render", "scene_index": idx})
                render_results[plan.get("scene_id", f"scene_{idx}")] = self._render(plan["render_ready_prompt"], opts, progress_cb=cb)
        ...
        if opts.do_export_renpy:
            ...
            if callable(compile_fn):
                renpy_chapter = compile_fn(processed_scenes, chapter_name=chapter, make_entry_label=True)
            else:
                ...
                for sc in processed_scenes:
                    ...
                    manifest.append(res)
                renpy_chapter = {"file": None, "chapter_label": chapter, "manifest": manifest, "script_text": None}
        ...
        self._emit(job_id, "complete", 1.0, {"stage": "done", "result": {"scenes": len(scenes)}})
        self.ctx.job_manager.complete(job_id, out)
        return out
