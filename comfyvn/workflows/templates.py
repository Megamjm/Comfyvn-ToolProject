...
TEMPLATES: Dict[str, Dict[str, Any]] = {
    ...
    "sprite_composite_basic": {
        ...
        "version": 1,
        "meta": {"description": "Load a background and up to three sprite layers, returning a composited image."},
        "inputs": {
        ...
        "nodes": [
            {"id": "bg", "type": "LoadImage", "params": {"filename": "${background}"}, "inputs": {}, "outputs": {"image": "bg.image"}},
            {"id": "left", "type": "LoadImage", "params": {"filename": "${sprite_left}"}, "inputs": {}, "outputs": {"image": "left.image"}},
            {"id": "center", "type": "LoadImage", "params": {"filename": "${sprite_center}"}, "inputs": {}, "outputs": {"image": "center.image"}},
            {"id": "right", "type": "LoadImage", "params": {"filename": "${sprite_right}"}, "inputs": {}, "outputs": {"image": "right.image"}},
            {
                "id": "composite",
                "type": "CompositeImage",
                "params": {},
                "inputs": {"background": "bg.image", "layers": "left.image|center.image|right.image"},
    ...
    "pose_blend_basic": {
        ...
        "version": 1,
        "meta": {"description": "Blend between two pose JSON files using the ComfyUI PoseInterpolator node."},
        "inputs": {
        ...
        "nodes": [
            {"id": "load_a", "type": "LoadPose", "params": {"filename": "${pose_a}"}, "inputs": {}, "outputs": {"pose": "load_a.pose"}},
            {"id": "load_b", "type": "LoadPose", "params": {"filename": "${pose_b}"}, "inputs": {}, "outputs": {"pose": "load_b.pose"}},
    ...
    "sprite_pose_composite": {
        ...
        "version": 1,
        "meta": {"description": "Blend poses, then composite sprites with optional pose metadata embedded."},
        "inputs": {
            "background": {"type": "string", "required": True},
        ...
        "outputs": {
            "image": "compose.out",
            "pose": "pose_interp.pose"
        },
        "nodes": [
            {"id": "pose_interp", "type": "PoseInterpolatorWorkflow", "params": {"pose_a": "${pose_a}", "pose_b": "${pose_b}", "blend": "${blend}"}, "inputs": {}, "outputs": {"pose": "pose_interp.pose"}},
            {"id": "bg2", "type": "LoadImage", "params": {"filename": "${background}"}, "inputs": {}, "outputs": {"image": "bg2.image"}},
            {"id": "sprite", "type": "LoadImage", "params": {"filename": "${sprite}"}, "inputs": {}, "outputs": {"image": "sprite.image"}},
            {
                "id": "compose",
                "type": "CompositeImage",
                "params": {},
